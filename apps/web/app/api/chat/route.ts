...
export async function POST(req: NextRequest) {
  try {
    ...
    const chatSession = await getOrCreateChatSessionByContentId(contentId, workspaceId, auth.userId)
    ...
    if (limits.maxAIRequestsPerDay) {
      const limitCheck = await canMakeAIRequest(auth.userId, subscriptionTier, limits.maxAIRequestsPerDay)
    ...
    await incrementAIRequestCount(auth.userId).catch((err) => {
