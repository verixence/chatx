...
export async function GET(req: NextRequest) {
  try {
    ...
    const contentCount = await getUserContentCount(auth.userId)
  ...
  try {
    ...
    if (subscription === 'freemium' && (user.subscription === 'pro' || user.subscription === 'enterprise')) {
      ...
      if (currentContentCount > freemiumLimit) {
        ...
        console.log(`User ${auth.userId} downgrading from ${user.subscription} to freemium with ${currentContentCount} content items (limit: ${freemiumLimit})`)
      }
    }
